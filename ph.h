...
// Returns 1 if the node is attached to the given heap, 0 otherwise
int pheap_node_attached(void *oph, void *opn);

// Detaches a node from the given heap without freeing it
// If the node is already detached then does nothing
void pheap_detach_node(void *oph, void *opn);

// Attaches/Inserts a node into the heap.  If it is
// already attached then this call does nothing
void pheap_attach_node(void *oph, void *opn);

// Releases (frees) the given node
// If the node is still attached, it detaches it first
void pheap_release_node(void *oph, void *opn);
...
// Sets key and data to that in the node if they are non-NULL
// First checks that node is attached
...
// If the node is attached, this performs a pheap_change_key()
void pheap_set_key(void *oph, void *opd, void *newkey);
