...
int
pheap_node_attached(void *oph, void *opn)
{
	register struct pheap *ph = (struct pheap *)oph;
...
// Detaches a node from the given heap without freeing it
// If the node is already detached, this does nothing
void
pheap_detach_node(void *oph, void *opn)
{
	register struct pheap *ph = (struct pheap *)oph;
	register struct heap *pn = (struct heap *)opn;
	...
	if (pn->prev || (ph->root == pn) || pn->sub || pn->next) {
		...
		} else {			// Is Root without children
			ph->root = NULL;
		}

		// Make sure pn references nothing else
		pn->next = NULL;
		pn->prev = NULL;
		pn->sub = NULL;
	}
...
// Attaches/Inserts a node from the heap without allocating it
// If the node is already attached, this does nothing
void
pheap_attach_node(void *oph, void *opn)
{
	register struct pheap *ph = (struct pheap *)oph;
	register struct heap *pn = (struct heap *)opn;
...
// Releases (frees) the given node
// If the node is attached, it detaches it, and then frees it
void
pheap_release_node(void *oph, void *opn)
{
	register struct pheap *ph = (struct pheap *)oph;
	register struct heap *pn = (struct heap *)opn;
...
pheap_change_key(void *oph, void *opd, void *newkey)
{
	...
	if (unlikely(oph == NULL) || unlikely(opd == NULL) || unlikely(ph->root == NULL))
		return;
...
// Changes the key of the given node that is a member of the given heap
// If the node is not attached, it just updates the key value only
// If the node is attached, this just acts as pheap_change_key()
void
pheap_set_key(void *oph, void *opn, void *newkey)
{
	register struct pheap *ph = (struct pheap *)oph;
	register struct heap *pn = (struct heap *)opn;
	...
	if (pn->prev || (ph->root == pn) || pn->sub || pn->next) {
		return pheap_change_key(ph, pn, newkey);
	}
	pn->key = newkey;
} // pheap_set_key
